...
Stream *Stream_Open(const String *path, bool write, bool fail)
{
	...
	if (file == NULL) {
	    if (fail) 
		Error(write ? EX_CANTCREAT : EX_NOINPUT,
...
Stream *Stream_OpenTemp(const String *path, bool write)
{
...
void WarnContentLength(Message *msg, int contLen, int bodyLen)
{
	...
	Warn("Message %s has been truncated: %d bytes missing",
	     String_CString(msg->tag), contLen - bodyLen);

    } else if (delta > 1 && contLen < bodyLen) {
	Warn("Message %s has been extended: %d bytes gained",
	     String_CString(msg->tag), bodyLen - contLen);

    } else if (gStrict) {
	Warn("Incorrect Content-Length: %d for message %s; using %d",
	     contLen, String_CString(msg->tag), bodyLen);
    }
}
...
void Mailbox_Append(Mailbox *mbox, Message *msg)
{
	...
	Error(EX_SOFTWARE, "Internal error: Trying to add tied message "
...
void Mailbox_Write(Mailbox *mbox, const String *destination)
{
    if (gVerbose) {
...
static int FindIllegalChar(String *str, bool controlOK, bool eightBitOK)
{
	...
	if (*chars == '\r' || *chars == '\n' || *chars == '\t')
	    continue;
	if (!controlOK && (iscntrl(*chars) || *chars == '\177'))
	    return i;
...
void CheckMailbox(Mailbox *mbox, bool strict, bool repair)
{
	...
	if (cllen != bodyLength) {
		...
		if (value != NULL) {
		    Warn("Content-Length: %s should be %d for message %s",
			 String_PrettyCString(value), bodyLength,
			 String_CString(msg->tag));
		} else if (strict) {
		    Warn("Missing Content-Length: header in message %s",
			 String_CString(msg->tag));
		}
	    }
	}
	...
	if (value == NULL) {
	    ...
	    if (value == NULL) {
		Warn("Missing From: header in message %s",
		     String_CString(msg->tag));
		...
		Warn("Missing From: header in message %s, but could use\n"
		     " %s: %s",
		     String_CString(msg->tag),
		     String_CString(source), String_CString(value));
	    }
	...
	if (value == NULL) {
	    ...
	    if (value == NULL) {
		Warn("Missing Date: header in message %s",
		     String_CString(msg->tag));
		...
		Warn("Missing Date: header in message %s, but could use\n"
		     " %s: %s",
		     String_CString(msg->tag), String_CString(source),
		     String_CString(value));
	    }
	...
	if (strict) {
		...
		if (value == NULL) {
		    Warn("Missing Message-ID: header in message %s",
			 String_CString(msg->tag));
		}
	    }
	}
	...
	if (strict) {
		...
		if (pos >= 0) {
		    Warn("Illegal character %s in header of message %s:\n"
			 " %s", 
		...
		if (pos >= 0) {
		    ...
		    Warn("Illegal character %s in body of message %s:\n %s%s",
			 Char_QuotedCString(String_CharAt(body, pos)),
			 String_CString(msg->tag), off == 0 ? "" : "...",
...
bool Message_Split(Message *msg, bool interactively)
{
	...
	if (!Parse_Newline(&parser, NULL) || !Parse_Newline(&parser, NULL))
	    Error(EX_SOFTWARE, "Internal error, couldn't parse double newline"
		  "in Message_Split");
	...
	if (Parse_FromSpaceLine(&parser, &line, NULL, NULL)) {
	    ...
	    Note("Found \"From \" line in body of message %s:\n %s",
		...
		while (Parse_Message(&parser, msg->mbox, &newMsg)) {
		    if (body != NULL) {
...
{
    if (*pIndex < Array_Count(args)) {
	Error(EX_OK, "Too many arguments");
	return false;
    }
...
MessageSet *MessageSetArg(String *arg, int maxMax)
{
	...
	!Parser_AtEnd(&parser)) {
	Error(EX_OK, "Malformed message set: %s", String_CString(arg));
	MessageSet_Free(set);
	return NULL;
    }
...
Message *GetMessageByNumber(Mailbox *mbox, int cur)
{
    ...
    if (msg == NULL)
	Error(EX_OK, "Message %d does not exist", cur);
...
void RunLoop(Mailbox *mbox, Array *commands)
{
	...
	switch (cmd) {
	  ...
	  case kCmd_Join:
	    ...
	    if (count == 0) {
		Error(EX_OK, "Please supply multiple messages to join");
	    } else {
